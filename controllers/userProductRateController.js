...
const createRatting = async (req, res) => {
    ...
    const isExistRatting = await userProductRateModel.findOne({userId:userId, productId: productId}).exec();
    ...
    if(!isExistRatting && isExistProduct) {
        ...
        console.log(ratting.id);
        const product = await productModel.updateOne({_id: productId}, {rate: [...isExistProduct.rate, ratting.id] }).catch( async (error) => {
